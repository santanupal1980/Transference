...
class EncoderLayer(nn.Module):  # defining sublayers of the encoder block
    ...
    def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, ahsize=None):
        ...
        self.attn = SelfAttn(isize, _ahsize, isize, num_head, dropout=attn_drop)  # mult-head attention
    ...
    def forward(self, inputs, mask=None):
        ...
        context = context + inputs  # residual

        _context = self.ff(context)  # Feed-forward
...
class Encoder(nn.Module):
    ...
    def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, xseql=512,
                 ahsize=None, norm_output=True):
        ...
        self.wemb = nn.Embedding(nwd, isize, padding_idx=0)  # word embeding
        self.pemb = PositionalEmb(isize, xseql, 0, 0)  # positional Embeding
        self.drop = nn.Dropout(dropout, inplace=True) if dropout > 0.0 else None
        # looping sub_layers N X Sublayers
        self.enc_blocks = nn.ModuleList(
            [EncoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize) for i in range(num_layer)])
